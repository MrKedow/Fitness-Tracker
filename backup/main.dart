  ...
  runApp(
    MultiProvider(
      providers: [
        ...
        Provider(create: (_) => NutstoreService()),
      ],
...
class ExcelCSVService {
  ...
  Future<List<WorkoutRecord>> importFromCSV(String filePath) async {
    try {
      ...
      // 用于跟踪当前日期和sessionNumber（处理留空的日期单元格）
...
class NutstoreService {
  ...
  Future<bool> connect({
    ...
    try {
      ...
      final request = await client.openUrl('PROPFIND', Uri.parse(_baseUrl));
      request.headers.set('Authorization', _getAuthHeader());
  ...
  Future<void> _ensureCloudFolder() async {
    try {
      final client = HttpClient();
      final folderUrl = '$_baseUrl/$_cloudFolder';
      
      // 先检查文件夹是否存在
      final checkRequest = await client.openUrl('PROPFIND', Uri.parse('$folderUrl/'));
      checkRequest.headers.set('Authorization', _getAuthHeader());
      checkRequest.headers.set('Depth', '1');
      ...
      // 创建文件夹
      final createRequest = await client.openUrl('MKCOL', Uri.parse(folderUrl));
      createRequest.headers.set('Authorization', _getAuthHeader());
  ...
  Future<bool> uploadFile(String localFilePath) async {
    try {
      ...
      if (!await file.exists()) {
        throw Exception('本地文件不存在: $localFilePath');
      }
      
      final remotePath = '$_baseUrl/$_cloudFolder/$_defaultFilename';
      ...
      final client = HttpClient();
      final request = await client.openUrl('PUT', Uri.parse(remotePath));
      request.headers.set('Authorization', _getAuthHeader());
  ...
  Future<bool> downloadFile(String savePath) async {
    try {
      ...
      final client = HttpClient();
      final request = await client.openUrl('GET', Uri.parse(remotePath));
      request.headers.set('Authorization', _getAuthHeader());
  ...
  Future<bool> checkFileExists() async {
    try {
      ...
      final client = HttpClient();
      final request = await client.openUrl('HEAD', Uri.parse(remotePath));
      request.headers.set('Authorization', _getAuthHeader());
...
class _MainScreenState extends State<MainScreen> {
  ...
  final DateTime firstWorkoutDate = DateTime(2025, 6, 9, 18, 29);
  ...
  void _loadData() async {
    ...
    final connected = await nutstore.loadSavedConfig();
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => _showCloudLoginDialog(context),
            tooltip: '云服务',
          ),
          if (_isLoading)
            const Padding(
